...
class MockVideosBloc extends MockBloc<VideosEvent, VideosState>
    implements VideosBloc {}
...
void main() {
  ...
  setUp(() {
    ...
    movieDetailBloc = MovieDetailBloc(
        mockGetMovieDetailUsecase, mockCastBloc, mockVideosBloc);
  });
  ...
  blocTest<MovieDetailBloc, MovieDetailState>(
    ...
    setUp: () {
      when(() => mockGetMovieDetailUsecase(1))
          .thenAnswer((_) async => const Right(movieDetailEntity));
    },
    build: () => movieDetailBloc,
    act: (bloc) => bloc.add(const MovieDetailLoadEvent(1)),
    expect: () => <MovieDetailState>[
      const MovieDetailLoaded(movieDetailEntity: movieDetailEntity)
    ],
  );
  ...
  blocTest<MovieDetailBloc, MovieDetailState>(
    ...
    act: (bloc) => bloc.add(const MovieDetailLoadEvent(1)),
    expect: () =>
        const <MovieDetailState>[MovieDetailError('Something went wrong!')],
  );
}
