...
import 'package:movie_app/domain/usecases/get_all_genres_usecase.dart';
...
void main() {
  ...
  blocTest<GenreBloc, GenreState>(
    ...
    setUp: () {
      ...
      when(() => mockGetAllGenresUsecase(noParams))
    ...
    expect: () => const <GenreState>[
      GenreLoading(),
      GenreLoaded(genreList: <GenreEntity>[])
    ],
  );
  ...
  blocTest<GenreBloc, GenreState>(
    ...
    expect: () => const <GenreState>[
      GenreLoading(),
      GenreError('Ops! Something went wrong')
    ],
  );
}
